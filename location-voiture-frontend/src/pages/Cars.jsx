      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
          {filteredCars.slice(0, 20).map((car, index) => (
            <div key={car.id} style={{ background: 'var(--card-background)', borderRadius: '24px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
              <div style={{ height: '220px' }}>
                <img 
                  src={getCarImage(car.brand, index)} 
                  alt={car.model} 
